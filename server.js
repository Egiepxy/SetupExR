...
app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'SetupExR Light MultiExchange V1.7', time: new Date().toISOString() }));
...
app.get('/api/market/recovery', async (req, res) => {
  try {
    ...
    if (!markets.has(market)) throw new Error('Mercado inválido');
    if (!['early', 'confirmed'].includes(mode)) throw new Error('Modo inválido');
    ...
    const settled = await Promise.all(candidates.map(async item => {
      try {
        ...
        const stage = score >= 8 ? 'Reversão confirmada' : score >= 6 ? 'Recuperação inicial' : score >= 4 ? 'Possível formação de fundo' : 'Continua fraca';
        return { ...item, score, stage, signals: checks.filter(x => x[1]).map(x => x[0]), missing: checks.filter(x => !x[1]).map(x => x[0]), rsi15: m15.rsi, rsi1h: m1h.rsi, volumeRatio: m15.volumeRatio, atrPct: m15.atrPct };
      } catch { return null; }
    }));
    const minimum = mode === 'confirmed' ? 8 : 6;
    const rows = settled.filter(Boolean).filter(x => x.score >= minimum).sort((a, b) => b.score - a.score || b.volume - a.volume).slice(0, 10);
...
app.listen(PORT, '0.0.0.0', () => console.log(`SetupExR Light MultiExchange V1.7 online na porta ${PORT}`));
