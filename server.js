...
app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'SetupExR Light Binance V1.5', time: new Date().toISOString() }));
...
app.get('/api/binance/volatility', async (_req, res) => {
  try {
    const tickers = await binanceMarket('/api/v3/ticker/24hr');
    ...
    const ranked = await Promise.all(liquid.map(async item => {
      try {
        const rows = await binanceMarket(`/api/v3/klines?symbol=${encodeURIComponent(item.symbol)}&interval=15m&limit=40`);
...
app.listen(PORT, '0.0.0.0', () => console.log(`SetupExR Light Binance V1.5 online na porta ${PORT}`));
